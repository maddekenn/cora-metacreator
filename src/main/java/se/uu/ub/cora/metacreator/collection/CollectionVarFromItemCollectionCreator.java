...
public class CollectionVarFromItemCollectionCreator implements ExtendedFunctionality {
    ...
    @Override
    public void useExtendedFunctionality(String authToken, SpiderDataGroup itemCollectionToCreateFrom) {
        this.authToken = authToken;
        this.itemCollectionToCreateFrom = itemCollectionToCreateFrom;
        extractIds();

        if(collectionVarDoesNotExist(idForCollectionVariable)) {
            SpiderDataGroup collectionVar = extractDataAndConstructCollectionVariable(itemCollectionToCreateFrom);
            createRecord("metadataItemCollection", collectionVar);
        }
    }
    ...
    private SpiderDataGroup extractDataAndConstructCollectionVariable(SpiderDataGroup itemCollectionToCreateFrom) {

        String nameInData = itemCollectionToCreateFrom.extractAtomicValue("nameInData");
        String dataDivider = DataCreatorHelper.extractDataDividerStringFromDataGroup(itemCollectionToCreateFrom);
        return constructCollectionVariable(idForCollectionVariable, nameInData, dataDivider);
    }

    private SpiderDataGroup constructCollectionVariable(String id, String nameInData, String dataDivider) {
        CollectionVariableConstructor constructor = new CollectionVariableConstructor();
        return constructor.
                constructCollectionVarWithIdNameInDataDataDividerAndRefCollection(id, nameInData, dataDivider, itemCollectionId);
    }
