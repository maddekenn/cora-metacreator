...
public class RecordTypeCreator implements ExtendedFunctionality {
    ...
    private void possiblyCreatePresentationGroups(SpiderDataGroup spiderDataGroup) {
    	...
    	String dataDivider = dataDividerGroup.extractAtomicValue("linkedRecordId");
    	String presentationOf = spiderDataGroup.extractAtomicValue("metadataId");
    	
    	String presentationId = spiderDataGroup.extractAtomicValue("presentationViewId");
    	if(recordDoesNotExistInStorage("presentationGroup", presentationId)){
	    	
	    	PGroupCreator presentationView = PGroupCreator.withIdDataDividerAndPresentationOf(presentationId, dataDivider, presentationOf);
	    	SpiderDataGroup pGroup = presentationView.createPresentationGroup();
